...
describe('QualityGateEngine Error Scenarios', () => {
    ...
    it('should handle null artifacts', async () => {
        const result = await engine.executeQualityGate('null-test', null);
        expect(result).toBeDefined();
        expect(result.passed).toBe(false);
        expect(result.violations.length).toBeGreaterThan(0);
    });
    it('should handle malformed artifact data', async () => {
        ...
        const result = await engine.executeQualityGate('malformed-test', malformedArtifacts);
        expect(result).toBeDefined();
...
//# sourceMappingURL=QualityGateEngine.test.js.map